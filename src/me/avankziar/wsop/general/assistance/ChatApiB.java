...
{	
	...
	public static TextComponent tctl(List<BaseComponent> s)
	{
		TextComponent t = ChatApiB.tc("");
		t.setExtra(s);
		return t;
	}
	...
	public static TextComponent click(String text, String caction, String cmd)
	{
		return click(text, ClickEvent.Action.valueOf(caction), cmd);
	}
	
	private static TextComponent click(String text, ClickEvent.Action caction, String cmd)
	{
		TextComponent msg = null;
		msg = tctl(text);
		msg.setClickEvent( new ClickEvent(caction, cmd));
	...
	public static TextComponent hover(String text, String haction, String hover)
	{
		return hover(text, HoverEvent.Action.valueOf(haction), hover);
	}
	
	private static TextComponent hover(String text, HoverEvent.Action haction, String hover)
	{
		String sepnewline = "~!~";
		TextComponent msg = null;
		ArrayList<BaseComponent> components = new ArrayList<>();
	...
	public static TextComponent clickHover(String text, String caction, String cmd,
			String haction, String hover)
	{
		return clickHover(text, ClickEvent.Action.valueOf(caction), cmd, HoverEvent.Action.valueOf(haction), hover);
	}
	
	private static TextComponent clickHover(String text, ClickEvent.Action caction, String cmd,
	...
	public static TextComponent generateTextComponent(String message)
	{
		...
		for(String word : space)
		{
			...
			if(word.contains(sepb))
			{
				...
				newtc = ChatApiB.tctl(function[0].replace(sepspace, " ")+" ");
				if(function.length == 2)
				{
					if(function[1].contains(idhover))
					{
						String[] at = function[1].split(sepw);
						ChatApiB.hoverEvent(newtc,HoverEvent.Action.valueOf(at[1]), at[2].replace(sepspace, " "));
					}
					if(function[1].contains(idclick))
					{
						String[] at = function[1].split(sepw);
						ChatApiB.clickEvent(newtc,ClickEvent.Action.valueOf(at[1]), at[2].replace(sepspace, " "));
					}
				}
